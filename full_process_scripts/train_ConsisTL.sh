...
switch_checkpoint=$5
loss_type=js
loss_args="--kd-weight $kd_weight --prior-tau $prior_tau --teacher-dir $teacher_dir --teacher-data $teacher_data --loss-type $loss_type"
loss_args_dir=kd_w-$kd_weight-prior_tau-$prior_tau-loss_type-$loss_type
save_dir=checkpoints-xkd/freq-$freq-lr-$lr-bsz-$bsz-clip-base-clean-warmup-$warmup-max_epoch-$max_epoch-seed-$seed-attn-dropout-$attn_dropout-act-dropout-$act_dropout-w_decay-$w_decay-$loss_args_dir
rm -r $save_dir
mkdir -p $save_dir
...
CUDA_VISIBLE_DEVICES=0,1,2,3 MKL_THREADING_LAYER=GNU python ../ConsisTL/train_transfer.py \
    $student_data \
    --arch transformer \
    --share-decoder-input-output-embed \
    --aux-src-lang de --aux-src $aux_src \
    $loss_args \
    --reset-optimizer \
    --reset-dataloader \
    --fp16 \
